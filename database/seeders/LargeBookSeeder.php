...
class LargeBookSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $extractTokens = function (string $title): array {
            $stop = [
                'the','and','of','a','an','to','in','on','for','with','without','from','by','about','into','over','under','between','beyond','within','across','at','as','is','are','be','being','been','or','not','it','its','their','his','her'
            ];
        ...
        $makeSynopsis = function (string $categoryName, string $title, array $keywords, $faker, array $focusTokens = []): string {
            ...
            $k1 = $pick();
            $k2 = \Illuminate\Support\Str::lower((string)($focusTokens[1] ?? $faker->randomElement($keywords)));
            $k3 = \Illuminate\Support\Str::lower((string)($focusTokens[2] ?? $faker->randomElement($keywords)));
        ...
        $blueprints = [
            ...
            'Biography' => [
                ...
                'keywords' => [
                    'Ada Lovelace','Alan Turing','Albert Einstein','Marie Curie','Isaac Newton','Leonardo da Vinci','Galileo Galilei','Nikola Tesla','Thomas Edison','Stephen Hawking','Katherine Johnson','Grace Hopper','Rosalind Franklin','Charles Darwin','Niels Bohr','Johannes Kepler','Michael Faraday','Max Planck','Richard Feynman','Carl Sagan','Neil Armstrong','Amelia Earhart','Sally Ride','Buzz Aldrin','Ernest Shackleton','Alexander the Great','Julius Caesar','Cleopatra','Genghis Khan','Napoleon Bonaparte','Winston Churchill','Nelson Mandela','Mahatma Gandhi','Martin Luther King Jr','Abraham Lincoln','George Washington','Theodore Roosevelt','Franklin D. Roosevelt','Queen Elizabeth I','Queen Victoria','Catherine the Great','Joan of Arc','Florence Nightingale','Vincent van Gogh','Pablo Picasso','Frida Kahlo','Claude Monet','Ludwig van Beethoven','Wolfgang Amadeus Mozart','Johann Sebastian Bach','Igor Stravinsky','William Shakespeare','Jane Austen','Charles Dickens','Leo Tolstoy','Mark Twain','Homer','Dante Alighieri','Miguel de Cervantes','Fyodor Dostoevsky','Virginia Woolf','Ernest Hemingway','George Orwell','Haruki Murakami','J.R.R. Tolkien','C.S. Lewis','Agatha Christie','Arthur Conan Doyle','J.K. Rowling','Chinua Achebe','Gabriel Garcia Marquez','James Joyce','T.S. Eliot','Maya Angelou','Emily Dickinson','Rabindranath Tagore','Rumi','Confucius','Socrates','Plato','Aristotle','Immanuel Kant','Friedrich Nietzsche','Søren Kierkegaard','Simone de Beauvoir','Hannah Arendt','Bertrand Russell','Sun Tzu','Murasaki Shikibu','Akira Kurosawa','Hayao Miyazaki','Yayoi Kusama','Steve Jobs','Bill Gates','Elon Musk','Jeff Bezos','Walt Disney','Coco Chanel','Malala Yousafzai','Oprah Winfrey','Serena Williams','Usain Bolt','Pelé','Diego Maradona','Lionel Messi','Michael Jordan','Roger Federer','Sachin Tendulkar','Cristiano Ronaldo','LeBron James'
                ],
                'templates' => [
            ...
            'Art' => [
                'keywords' => ['painting','sculpture','impressionism','modern art','renaissance','photography','architecture','design','museums','color theory','typography','composition','printmaking','installation','street art'],
                'templates' => [
            ...
            'Health' => [
                'keywords' => ['nutrition','fitness','mental health','sleep','meditation','yoga','cardio','strength','habits','longevity','mobility','recovery','hydration','mindfulness','stress'],
                'templates' => [
            ...
            'Travel' => [
                'keywords' => ['Japan','Alps','Bali','Istanbul','New York','Paris','Sahara','Amazon','Himalaya','Silk Road','Mediterranean','Patagonia','Iceland','Rome','Kyoto'],
                'templates' => [
            ...
            'Cooking' => [
                'keywords' => ['Italian','French','Japanese','Indonesian','baking','vegetarian','grilling','sauces','spices','desserts','noodles','rice','sourdough','salads','soups'],
                'templates' => [
            ...
            'Self-Help' => [
                'keywords' => ['habits','mindset','productivity','focus','discipline','communication','confidence','leadership','creativity','decision-making','resilience','emotion','learning','journaling','systems'],
                'templates' => [
            ...
            'Poetry' => [
                'keywords' => ['love','night','sea','wind','silence','memory','time','rain','light','shadow','city','river','garden','stone','winter'],
                'templates' => [
        ...
        foreach (Category::orderBy('name')->get() as $category) {
            ...
            foreach ($existingBooks as $eb) {
                ...
                $eb->synopsis = $makeSynopsis($category->name, (string)$eb->title, $bp['keywords'], $faker, $focusTokens);
                $eb->save();
            }
