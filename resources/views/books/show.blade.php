@extends("layouts.base", ["title" => $book->title, "metaDescription" => \Illuminate\Support\Str::limit(strip_tags($book->synopsis), 200)])

@section("head")
  <meta property="og:title" content="{{ $book->title }}" />
  <meta
    property="og:description"
    content="{{ \Illuminate\Support\Str::limit(strip_tags($book->synopsis), 200) }}"
  />
  ...
  @php
    $jsonLd = json_encode(
      [
        ...
        "description" => (string) \Illuminate\Support\Str::limit(strip_tags((string) $book->synopsis), 500),
        "genre" => (string) (optional($book->category)->name ?? "Book"),
        "url" => (string) url()->current(),
      ],
      JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE,
    );
    $jsonLd = str_replace(['"context"', '"type"'], ['"@context"', '"@type"'], $jsonLd);
  @endphp
...
@section("content")
  ...
  <div class="max-w-4xl mx-auto">
    <a
      href="{{ url()->previous() ?: route("books.catalog") }}"
    ...
    <div class="mt-4 glass-card p-6">
      ...
      <p class="mb-1 muted">
        Author:
        <span class="text-white">{{ $book->author ?? "Unknown" }}</span>
      </p>
      <p class="mb-4 muted">
        Category:
        <span class="text-white">{{ $book->category->name ?? "-" }}</span>
      </p>
